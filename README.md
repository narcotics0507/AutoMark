...
这是一款强大的 Chrome 浏览器扩展，利用先进的 AI 模型（OpenAI GPT-4, Google Gemini, DeepSeek 等）智能分析、分类并清理您的浏览器书签。
...
### 2. 选择文件夹
*   在文件夹列表中勾选您希望整理的目录（例如“书签栏”或“其他书签”）。
*   **选项**：
...
### 3. 分析与生成计划
*   AI 将扫描您的书签并生成一份整理计划。
*   如果是“仅检测死链”模式，系统将快速扫描链接状态。
*   在此过程中，您可以随时点击 **“停止分析”**，并通过 **“返回上一步”** 重新选择。
...
*   **备份建议**：虽然我们提供了审查机制，但在进行大规模整理前，强烈建议您先导出当前书签作为备份 (`chrome://bookmarks/` -> 右上角菜单 -> 导出书签)。
